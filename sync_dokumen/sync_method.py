...
@frappe.whitelist()
def enqueue_sync_document_inv(doc, method):
				...
				if customer_check:
					for row_sync in customer_check:
						if row_sync[0] == doc.supplier:
							...
							enqueue("sync_dokumen.sync_method.sync_po_so", doc=doc,sync_doc=sync_form,customer=row_sync[1])
...
def sync_po_so(doc,sync_doc,customer):
	...
	clientroot = FrappeClient(sync_doc.name, "test@test.com", "asd123")
	sumber = sync_doc.name.replace("http://","")
...
def sync_received_qty(doc,method):
	...
	tujuan_server =  frappe.db.sql("""  select sf.`parent` from `tabSync Form Customer Settings` sf
		where sf.`sumber_supplier` ="{}"  """.format(doc.supplier), as_dict = 1)
	...
	for d in data_po:
		for t in tujuan_server :
			server = t.parent.replace("https://","")
			server = server.replace("/","")
			enqueue("sync_dokumen.sync_method.enqueue_sync_received_qty", tujuan_server=server,po=d.po,prec=doc.name, asal_server = asal_server)

@frappe.whitelist()
def enqueue_sync_received_qty(tujuan_server,po, prec, asal_server):

	os.chdir("/home/frappe/frappe-bench")
	os.system(""" bench --site {0} execute sync_dokumen_support.sync_method_support.update_received_qty --args "['{1}','{2}','{3}']" """.format(tujuan_server, po, prec, asal_server))
...
def cancel_sync_received_qty(doc,method):
	...
	for d in data_po:
		for t in tujuan_server :
			server = t.parent.replace("https://","")
			server = server.replace("/","")
			enqueue("sync_dokumen.sync_method.enqueue_cancel_sync_received_qty", tujuan_server=server,po=d.po,prec=doc.name, asal_server = asal_server)

@frappe.whitelist()
def enqueue_cancel_sync_received_qty(tujuan_server,po, prec, asal_server):

	os.chdir("/home/frappe/frappe-bench")
	os.system(""" bench --site {0} execute sync_dokumen_support.sync_method_support.cancel_update_received_qty --args "['{1}','{2}','{3}']" """.format(tujuan_server, po, prec, asal_server))
...
def enqueue_check_form(doc, method):
	enqueue("sync_dokumen.sync_method.check_form", doc=doc)
	# except:
...
def check_form(doc):
	...
	try:
		clientroot = FrappeClient(sync_doc.name, "sync_user@antman.com", "asd123")
		...
		for row in doc.supplier_list:
			try:
				...
				if not docu_tujuan:
					frappe.db.sql(""" UPDATE `tabSync Form` SET message = "Success to Connect to Destination Server. Customer {} don't exists in Destination Server."  WHERE name = "{}" """.format(row.tujuan_supplier, doc.name))
			except:
				frappe.db.sql(""" UPDATE `tabSync Form` SET message = "Success to Connect to Destination Server. Customer {} don't exists in Destination Server."  WHERE name = "{}" """.format(row.tujuan_supplier, doc.name))
	except:
